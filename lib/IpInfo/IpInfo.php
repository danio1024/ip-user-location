...
class IpInfo
{
    ...
    private $additionalCurlOptions = array();
     ...
     *
     */
    function __construct($apiKey, $format = 'raw', $version = 'v3', array $additionalCurlOptions = null)
    {
        $this->apiKey = $apiKey;
        $this->apiFormat = $format;
        $this->apiVersion = $version;
        if ($additionalCurlOptions) {
    ...
    private function execute($ip, $endpoint)
    {
        ...
        $handle = $this->getConfiguredHandle($url);
     ...
     * @return resource
     */
    protected function getConfiguredHandle($url)
    {
        // Initialise CURL
        $handle = curl_init();
        ...
        if ($this->additionalCurlOptions) {
            curl_setopt_array($handle, $this->additionalCurlOptions);
        }
